 ...
 */
export function Service<T extends ClassDeclaration>(options: ServiceOptions<T> = {})
{
...
function getAncestors(target: ClassDeclaration): ClassDeclaration[]
{
...
function registerService<T extends ClassDeclaration>(
    classDeclaration: T,
...
): void
{
    ...
    for (let i = 0; i < options.tags.length; i++) {
        const tag = options.tags[i] as string | ServiceTag;

        if (typeof tag === 'string') {
            options.tags[i] = { name: tag, priority: 0 } as ServiceTag;
        } else {
